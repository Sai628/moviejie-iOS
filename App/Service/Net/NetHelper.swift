...
class NetHelper
{
                                                ...
                                                onError: NetError, successHandler: NetSuccessHandler)
    {
        ...
        if !tryToHandleError(jsonObject, onError: onError)
        {
            ...
            {
                try? stringStorage.setObject(value, forKey: cacheKey, expiry: .date(Date().addingTimeInterval(Constant.CACHE_EXPIRE_TIME)))
            }
        }
    }
                        ...
                        onError: NetError, onFailure: NetFailure, successHandler: @escaping NetSuccessHandler)
    {
        ...
        if cacheResult, let entry = try? stringStorage.entry(forKey: url)
        {
            if entry.expiry.isExpired {
                log.info("找到缓存数据, 不过已过期. 将重新请求")
                try? stringStorage.removeObject(forKey: url)  // 将当前过期的缓存删除掉
            } else {
                log.info("找到缓存数据.")
